...
- `HIVEMIND_WORKER_PRIORITY_LABELS`: comma-separated label names that worker loops prefer before the default smallest-eligible-issue fallback, for example `bug,help wanted`. Workers still skip issues with active branches, open PRs, or out-of-lane ownership before applying label priority.

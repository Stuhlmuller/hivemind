...
Use this prompt for the main development worker loop.
...
1. The worker loop owns one active issue branch at a time in its dedicated worktree.
...
4. If the branch's PR is still open, continue that issue until its PR is updated and ready for the beekeeper. Do not start a second issue.
...
9. Leave merging to the beekeeper loop even if the checks are already green.
...
1. Use bounded subagents whenever delegation would materially help an issue run.
2. Prefer an explorer-style subagent first to trace the relevant codepaths, tests, and likely file ownership.
3. After the main loop owns the issue branch and plan, you may spawn multiple worker-style subagents for disjoint slices such as tests, docs, or separate file groups.
